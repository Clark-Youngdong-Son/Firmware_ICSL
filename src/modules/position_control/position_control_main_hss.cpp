...
MulticopterPositionControl::poll_subscriptions()
{
	...
	if (updated) {
		...
		//// XXX Enabled for manual mode only!!
...
MulticopterPositionControl::limit_pos_sp_offset()
{
	...
	//// XXX this seems to be true in offboard mode
...
MulticopterPositionControl::control_non_manual(float dt)
{
	...
	if (_control_mode.flag_control_offboard_enabled) {
		/* offboard control */
		control_offboard(dt); //// generates _pos_sp or _vel_sp
		_mode_auto = false;
	...
	if (_pos_sp_triplet.current.valid) {
		...
		if (_control_mode.flag_control_position_enabled && 
			...
			if (fabsf(_pos_sp_triplet.current.vx) <= FLT_EPSILON &&
			    ...
			    _local_pos.xy_valid) {
				//// for small velocities, stay hold on
				...
				//// for accecptable velocities, use current velocity as setpoint
				if (_pos_sp_triplet.current.velocity_frame == position_setpoint_s::VELOCITY_FRAME_LOCAL_NED) {
					...
					//// velocities in global frame, so yaw compensation required
		...
		if (_control_mode.flag_control_altitude_enabled && _pos_sp_triplet.current.alt_valid) {
			...
			if (fabsf(_pos_sp_triplet.current.vz) <= FLT_EPSILON &&
			    _local_pos.z_valid) {
				//// for small delta Z, stayv hold on
				if (!_hold_offboard_z) {
					_pos_sp(2) = _pos(2);
				...
				//// for acceptable velocity, use current value as setpoint
		...
		if (_pos_sp_triplet.current.yaw_valid) {
			...
			//// generate setpoint yaw from yawspeed
...
MulticopterPositionControl::do_control(float dt)
{
	...
	_run_pos_control = true;
	_run_alt_control = true;
	mavlink_log_info(&_mavlink_log_pub, "[pos_control] control loop");
	...
	if (_control_mode.flag_control_manual_enabled) {
		...
		_hold_offboard_z = false;
		_isInitialized = false;
		mavlink_log_info(&_mavlink_log_pub, "[pos_control] usual manual mode");
	} else {
		//// ssk, filter initialize
...
//// XXX this is base position control
//// XXX in this step, _vel_sp is computed from _pos_sp if _run_pos_control is true
void
MulticopterPositionControl::control_position(float dt)
{
	if (_control_mode.flag_control_offboard_enabled) {
		mavlink_log_info(&_mavlink_log_pub, "[pos_control] offboard mode");
		//// I want to modify code only in offboard mode
		//// DOB implementation
		//// ssk 
		math::Vector<3> ge3;
		ge3.zero();
...
//		 (double)temp(0), (double)temp(1), (double)temp(2));
		...
		mavlink_log_info(&_mavlink_log_pub, "[pos_control] manual mode");
		//// I don't want to modify code in manual mode...
